...
class FingerPaintImageView @JvmOverloads constructor(context: Context,
    ...
    private var deletePaths: MutableList<Pair<Path, Paint>> = mutableListOf()
...
//        val yPos = event.y.coerceIn(yTranslation, yTranslation + sourceBitmap.intrinsicHeight * scale)
        ...
        val xPos = event.x.coerceIn(xTranslation, xTranslation + width)
        val yPos = event.y.coerceIn(yTranslation, yTranslation + height)
    ...
    fun undo() {
        if (paths.size > 0) {
            deletePaths.add(paths[paths.lastIndex])
        }
    ...
    fun redo() {
        if (deletePaths.size > 0) {
            paths.add(deletePaths[deletePaths.lastIndex])
            deletePaths.takeIf { it.isNotEmpty() }?.removeAt(deletePaths.lastIndex)
    ...
    fun save() {
        val file = File(Environment.getExternalStorageDirectory(), "smart.jpg")
        val fileOutputStream = FileOutputStream(file)
        brushBitmap?.compress(Bitmap.CompressFormat.PNG, 100, fileOutputStream)
